...
task main ()
{
	...
	while (true) {
		...
		if (vexRT[Btn7D] == 1) {
			...
			if (motor[seederopen] < maxServoPos) {
				motor[seederopen] = motor[seederopen] + servoIncrement;
				motor[seederclose] = motor[seederclose] - servoIncrement;
			}
		}
		...
		if (vexRT[Btn7U] == 1) {
			...
			if (motor[seederopen] > maxServoNeg ) {
				motor[seederopen] = motor[seederopen] - servoIncrement;
				motor[seederclose] = motor[seederclose] + servoIncrement;
			}
		}
